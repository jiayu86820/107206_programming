...
import org.secuso.privacyfriendlynotes.code_old.DbContract.CategoryEntry;
import org.secuso.privacyfriendlynotes.code_old.DbContract.NoteEntry;
import org.secuso.privacyfriendlynotes.code_old.DbContract.NotificationEntry;
...
public class DbAccess {
     ...
     */
    public static int addNote(Context c, String name, String content, int type, int category,String tag){
        DbOpenHelper dbHelper = new DbOpenHelper(c);
     ...
     */
    public static void updateNote(Context c, int id, String name, String content,String tag, int category) {
        DbOpenHelper dbHelper = new DbOpenHelper(c);
        SQLiteDatabase db = dbHelper.getWritableDatabase();
     ...
     * @param c the current context
     * @return A {@link Cursor} over all the notes
     */
    ...
    public static Cursor getCursorFindNotes(Context c , String tag)  {
        DbOpenHelper dbHelper = new DbOpenHelper(c);
        SQLiteDatabase db = dbHelper.getReadableDatabase();

        String[] projection = {NoteEntry.COLUMN_ID, NoteEntry.COLUMN_TYPE, NoteEntry.COLUMN_NAME, NoteEntry.COLUMN_CONTENT, NoteEntry.COLUMN_TAG, NoteEntry.COLUMN_PHOTO};
        String selection = NoteEntry.COLUMN_TAG + " =?  OR "  + NoteEntry.COLUMN_CONTENT + " LIKE ?  OR " + NoteEntry.COLUMN_NAME + " LIKE ?  ";
        String[] selectionArgs =  { tag.trim()};
        return db.query(NoteEntry.TABLE_NAME,   // Table name
                projection,                     // SELECT
                selection,                           // Columns for WHERE,
                new String[] {tag,'%'+tag+'%','%'+tag+'%'}, // Values for WHERE
                null,                           // Group
                null,                           // Filter by Group
                null);                     // Sort Order
    }
